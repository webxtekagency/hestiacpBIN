...
check_wordpress() {
    ...
    if [ -f "$wpconfig" ]; then
        ...
        if is_more_permissive_than "$perms" "640"; then
            result_fail "B22" "wp-config.php is ${perms} (should be ≤640) [${domain}]"
        else
            result_pass "B22" "wp-config.php permissions OK (${perms}) [${domain}]"
        fi
    fi
    ...
    if [ -f "$debuglog" ]; then
        ...
        if is_more_permissive_than "$dperms" "640"; then
            result_fail "B23" "debug.log exists and is ${dperms} (should be ≤640 or removed) [${domain}]"
        else
            result_warn "B23" "debug.log exists (consider removing in production) [${domain}]"
        fi
    else
        result_pass "B23" "No debug.log file [${domain}]"
    fi
    ...
    if [ -f "$wpconfig" ]; then
        ...
        if [ "${file_edit:-0}" -gt 0 ] 2>/dev/null; then
            result_pass "B24" "File editing is disabled (DISALLOW_FILE_EDIT) [${domain}]"
        else
            result_warn "B24" "File editing not disabled in wp-config.php [${domain}]"
        fi

        local table_prefix
        table_prefix=$(grep "table_prefix" "$wpconfig" 2>/dev/null | head -1 | sed "s/.*'\([^']*\)'.*/\1/")
        if [ "$table_prefix" = "wp_" ]; then
            result_warn "B27" "Default table prefix 'wp_' (recommend changing) [${domain}]"
        else
            result_pass "B27" "Non-default table prefix (${table_prefix}) [${domain}]"
        fi
    fi
    ...
    if [ -d "${docroot}/wp-content/uploads" ]; then
        ...
        if [ -f "$uploads_htaccess" ] && grep -qi "php" "$uploads_htaccess" 2>/dev/null; then
            result_pass "B28" "PHP execution blocked in uploads via .htaccess [${domain}]"
        elif $nginx_deny; then
            result_pass "B28" "PHP execution blocked in uploads via Nginx [${domain}]"
        else
            result_fail "B28" "PHP execution NOT blocked in wp-content/uploads [${domain}]"
        fi
    fi
}
