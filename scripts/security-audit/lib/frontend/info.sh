...
check_info_disclosure() {
    ...
    if [ "$env_status" = "200" ]; then
        result_critical "F18" "/.env is accessible (HTTP 200) [${domain}]"
    else
        result_pass "F18" "/.env properly blocked (HTTP ${env_status:-timeout}) [${domain}]"
    fi

    local git_status
    git_status=$(curl -sI -m 5 "${url}/.git/config" 2>/dev/null | head -1 | awk '{print $2}')
    if [ "$git_status" = "200" ]; then
        result_critical "F19" "/.git/config is accessible (HTTP 200) [${domain}]"
    else
        result_pass "F19" "/.git/config properly blocked (HTTP ${git_status:-timeout}) [${domain}]"
    fi
