...
install_script "$SRC_DIR/backup-users-custom/v-backup-users-custom" "v-backup-users-custom"
install_config "$SRC_DIR/backup-users-custom/backup-custom.conf.sample" "/etc/hestiacp-backup-custom.conf"

echo "v-clean-garbage:"
install_script "$SRC_DIR/clean-garbage/v-clean-garbage" "v-clean-garbage"
install_config "$SRC_DIR/clean-garbage/clean-garbage.conf.sample" "/etc/hestiacp-clean-garbage.conf"

echo "v-github-mirror:"
install_script "$SRC_DIR/github-mirror/v-github-mirror" "v-github-mirror"
# Main config contains user repos — never overwrite it
if [ ! -f "/etc/hestiacp-github-mirror.conf" ]; then
    install_config "$SRC_DIR/github-mirror/github-mirror.conf.sample" "/etc/hestiacp-github-mirror.conf"
else
    echo "  -> [INFO] Config already exists (skipped): /etc/hestiacp-github-mirror.conf"
fi
install_config "$SRC_DIR/github-mirror/github-mirror.settings.sample" "/etc/hestiacp-github-mirror.settings"

echo "v-system-report:"
install_script "$SRC_DIR/system-report/v-system-report" "v-system-report"
install_config "$SRC_DIR/system-report/system-report.conf.sample" "/etc/hestiacp-system-report.conf"
...
CRONHEADER
    ...
    # v-system-report: every Monday at 08:00 (start-of-week report)
    ...
    # v-github-mirror: weekly forced notification (Sunday 10:00)
